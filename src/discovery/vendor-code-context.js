...
if (terms.length === 0) {
  terms.push("webGetFiles", "webGetDownloadCenterHub", "rpcRequest", "httpRequest");
}

async function main() {
  const text = await fetchText(scriptUrl);
  const decoder = buildDecoder(text);
  const code = stripStringTable(text);
  const decoded = decodeCalls(code, decoder);

  for (const term of terms) {
    console.log(`\n\n######## ${term} ########`);
    printContexts(decoded, term);
  }
}

function stripStringTable(text) {
  const marker = "const _0x223d5f=[";
  const start = text.indexOf(marker);
  if (start === -1) return text;
  const literal = extractStringArray(text);
  return text.slice(0, start) + "const _0x223d5f=[];" + text.slice(start + "const _0x223d5f=".length + literal.length);
}
...
function printContexts(text, term) {
  ...
  while (index !== -1 && count < 12) {
    ...
    const end = Math.min(text.length, index + term.length + 1800);
    const context = text
      .slice(start, end)
      .replace(/\\u002F/g, "/")
      .replace(/\\\//g, "/")
...
function decodeCalls(text, decoder) {
  return text.replace(/(?:a8_0x350a|a8_0x2d37ec|_0x[0-9a-f]+)\((0x[0-9a-f]+)\)/gi, (match, hex) => {
    try {
